            ...
            <div class="p-6">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Manage admin and employee accounts</p>
                    </div>
                    <button wire:click="openModal" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-200 flex items-center">
                            ...
                            @forelse($users as $user)
                                <tr class="hover:bg-gray-50 transition duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('admin.user-detail', $user->id) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-900">
                                            {{ $user->name }}
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($user->wallet)
                                            <span class="text-sm font-semibold {{ $user->wallet->balance < 0 ? 'text-red-600' : 'text-green-600' }}">
                                                ₹{{ number_format($user->wallet->balance, 2) }}
                                            </span>
                                        @else
                                            <span class="text-sm text-gray-400">No wallet</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.user-detail', $user->id) }}" class="text-blue-600 hover:text-blue-900 mr-3 transition duration-200" title="View Details">
                                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
                                        </a>
                                        <button wire:click="edit({{ $user->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3 transition duration-200" title="Edit">
                                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </button>
                                        <button wire:click="confirmDelete({{ $user->id }})" class="text-red-600 hover:text-red-900 transition duration-200" {{ $user->id === auth()->id() ? 'disabled' : '' }} title="Delete">
                                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm mt-1">Click "Add User" to create your first user</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mb-4">
                                <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                                    Password @if(!$isEditing)<span class="text-red-500">*</span>@else<span class="text-gray-500">(leave blank to keep current)</span>@endif
                                </label>
                                <input type="password"
                                       wire:model="password"
                                       id="password"
                            ...
                            <!-- Initial Balance (only for new users) -->
                            @if(!$isEditing)
                                <div class="mb-4">
                                    ...
                                    <div class="mt-1 relative rounded-md shadow-sm">
                                        ...
                                        <input type="number"
                                               ...
                                               step="0.01"
                                               min="0"
                                               class="pl-7 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('initial_balance') border-red-500 @enderror"
                                               placeholder="0.00">
                                    </div>
                                    @error('initial_balance')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Optional: Set the initial wallet balance for this user</p>
                                </div>
                            @endif
                        </div>
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Are you sure you want to delete this user? This action cannot be undone. All associated data including wallet and transactions will be permanently removed.
                                    </p>
                                </div>
                            </div>
                        </div>
